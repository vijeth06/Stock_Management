  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      drawHeader(
        ...
        "KONGU ENGINEERING COLLEGE",
        "",
        "FINANCIAL VALUATION & DEPRECIATION REPORT",
        "",
        reportData.year || new Date().getFullYear()
      );
      ...
      const infoPairs = [
        ["Report ID", reportData.reportId || `FIN-${Date.now()}`],
        ["Generated By", reportData.auditOfficer || "Administrator"],
        ["Report Date", formatDateDDMMYYYY(reportData.generatedAt || new Date())],
        ["Financial Year", `FY ${reportData.year || new Date().getFullYear()}`],
        ["Depreciation Method", reportData.depreciationMethod || "Straight-Line"],
      ];
      y = drawMetaGrid(doc, infoPairs, y);
      ...
      sectionHeading(doc, "Portfolio Financial Summary", 1);

      const kpis = [
        { label: "Total Asset Count", value: String(reportData.totalAssets || reportData.assetCount || 0) },
        { label: "Total Portfolio Value", value: formatCurrency(reportData.totalPurchaseValue || 0) },
        { label: "Net Book Value", value: formatCurrency(reportData.netBookValue || 0) },
        { label: "Total Depreciation", value: formatCurrency((reportData.totalPurchaseValue || 0) - (reportData.netBookValue || 0)) },
      ];
      drawKpis(doc, kpis, 2);

      // Asset Financial Ledger Table
      sectionHeading(doc, "Asset Financial Ledger", 2);

      const assetsList = reportData.assetsList || [];
      if (assetsList.length === 0) {
        doc.fillColor(COLORS.muted).fontSize(9).font("Helvetica").text("No assets found.", MARGIN_LEFT, doc.y);
        doc.moveDown(0.8);
      } else {
        const headers = ["Asset ID", "Name", "Dept", "Purchase Value", "Net Book Value", "Status"];
        const colWidths = [
          Math.floor(CONTENT_WIDTH * 0.15),
          Math.floor(CONTENT_WIDTH * 0.28),
          Math.floor(CONTENT_WIDTH * 0.10),
          Math.floor(CONTENT_WIDTH * 0.17),
          Math.floor(CONTENT_WIDTH * 0.17),
        ...
        const rows = assetsList.map(a => {
          ...
          return [
            safeValue(a.assetId),
            safeValue(a.name, "Asset"),
            safeValue(a.department),
            formatCurrency(val),
            formatCurrency(nbv),
            safeValue(a.status, "Active"),
          ];
        });
        y = drawWideSummaryTable(doc, headers, colWidths, rows, doc.y, 7, 16);
      }

      // Signature block
      ensureSpace(doc, 80);
      y = doc.y + 20;
      drawSignatures(doc, y, "Finance Officer", "Registrar", "Principal");
      ...
      doc
        ...
        .fontSize(8)
        .font("Helvetica-Oblique")
        .text("ChainTrack Asset Management System — Financial Valuation Report",
          MARGIN_LEFT, PRINTABLE_BOTTOM - 20, { width: CONTENT_WIDTH, align: "center" });
...
// ===========================================================================
// VALUATION TAB PDF — Department-wise asset valuation with status breakdown
// ===========================================================================
async function generateValuationPdf(reportData) {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ margin: 0, size: "A4", autoFirstPage: true });
      const buffers = [];
      ...
      drawHeader(
        ...
        "KONGU ENGINEERING COLLEGE",
        "",
        "DEPARTMENT ASSET VALUATION REPORT",
        "",
        reportData.year || new Date().getFullYear()
      );
      ...
      const infoPairs = [
        ["Report ID", reportData.reportId || `VAL-${Date.now()}`],
        ["Generated By", reportData.auditOfficer || "Administrator"],
        ["Report Date", formatDateDDMMYYYY(reportData.generatedAt || new Date())],
        ["Financial Year", `FY ${reportData.year || new Date().getFullYear()}`],
      ];
      ...
      // Executive Summary KPIs
      sectionHeading(doc, "College Asset Overview", 1);

      const valuationData = reportData.valuationData || {};
      const deptEntries = Object.entries(valuationData).filter(([_, d]) => typeof d === "object");
      const totalAssets = deptEntries.reduce((sum, [, d]) => sum + (d.totalAssets || 0), 0);
      const totalPurchaseValue = deptEntries.reduce((sum, [, d]) => sum + (d.totalPurchaseValue || 0), 0);
      const totalNbv = deptEntries.reduce((sum, [, d]) => sum + (d.netBookValue || 0), 0);
      const totalActive = deptEntries.reduce((sum, [, d]) => sum + (d.activeAssets || 0), 0);
      const totalMaintenance = deptEntries.reduce((sum, [, d]) => sum + (d.maintenanceAssets || 0), 0);
      const totalCondemned = deptEntries.reduce((sum, [, d]) => sum + (d.condemnedAssets || 0), 0);
      const totalDisposed = deptEntries.reduce((sum, [, d]) => sum + (d.disposedAssets || 0), 0);
      ...
      const kpis = [
        ...
        { label: "Active Assets", value: String(totalActive) },
        { label: "Under Maintenance", value: String(totalMaintenance) },
        { label: "Condemned / Disposed", value: String(totalCondemned + totalDisposed) },
      ];
      drawKpis(doc, kpis, 3);

      // Department Valuation Table
      sectionHeading(doc, "Department Valuation Breakdown", 2);

      if (deptEntries.length === 0) {
        doc.fillColor(COLORS.muted).fontSize(9).font("Helvetica").text("No valuation data available.", MARGIN_LEFT, doc.y);
        doc.moveDown(0.8);
      } else {
        const headers = ["Department", "Assets", "Purchase Value", "Net Book Value", "Active", "Maintenance", "Condemned", "Disposed"];
        const colWidths = [
          Math.floor(CONTENT_WIDTH * 0.20),
          Math.floor(CONTENT_WIDTH * 0.08),
          Math.floor(CONTENT_WIDTH * 0.16),
          Math.floor(CONTENT_WIDTH * 0.16),
          Math.floor(CONTENT_WIDTH * 0.10),
          Math.floor(CONTENT_WIDTH * 0.10),
          Math.floor(CONTENT_WIDTH * 0.10),
        ...
        const rows = deptEntries.map(([key, dept]) => [
          safeValue(dept.name || key),
      ...
      sectionHeading(doc, "Asset Status Summary by Department", 3);

      if (deptEntries.length > 0) {
        const statusHeaders = ["Department", "Active %", "Maintenance %", "Condemned %", "Disposed %"];
        const statusColWidths = [
          Math.floor(CONTENT_WIDTH * 0.30),
          Math.floor(CONTENT_WIDTH * 0.175),
          Math.floor(CONTENT_WIDTH * 0.175),
          Math.floor(CONTENT_WIDTH * 0.175),
          Math.floor(CONTENT_WIDTH * 0.175),
        ];
        const statusRows = deptEntries.map(([key, dept]) => {
          const total = dept.totalAssets || 1;
          const activePct = ((dept.activeAssets || 0) / total * 100).toFixed(0);
          const maintPct = ((dept.maintenanceAssets || 0) / total * 100).toFixed(0);
          const condPct = ((dept.condemnedAssets || 0) / total * 100).toFixed(0);
          const dispPct = ((dept.disposedAssets || 0) / total * 100).toFixed(0);
          return [
            safeValue(dept.name || key),
            `${activePct}%`,
            `${maintPct}%`,
        ...
        y = drawWideSummaryTable(doc, statusHeaders, statusColWidths, statusRows, doc.y, 7, 16);
      }
      ...
      doc
        ...
        .fontSize(8)
        .font("Helvetica-Oblique")
        .text("ChainTrack Asset Management System — Department Valuation Report",
          MARGIN_LEFT, PRINTABLE_BOTTOM - 20, { width: CONTENT_WIDTH, align: "center" });
